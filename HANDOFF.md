...
- **Status:** Phase 1 **complete**; Phase 2 in progress. Done: the **LLM
  gateway** (SPIFFE mTLS; the agent holds no model credential) and **secrets out
  of git/manifests**. Next: observability, policy lifecycle, supply-chain
  signing, operator guide.
...
- **Next: Phase 2** — production hardening. Done so far: the **LLM gateway**
  (`app/gateway/`, SPIFFE mTLS) and **secrets out of git** (`.env` +
  `docs/secrets.md`). Remaining: OpenTelemetry, policy lifecycle, SBOM/signing,
  and the operator guide.

## Immediate next task

**Phase 2, next: observability.** Add OpenTelemetry traces spanning
agent → gateway → tools → policy, with identity attributes on every span, plus
metrics and a trace viewer. Then policy lifecycle, supply-chain signing, and the
operator guide (see [`docs/roadmap.md`](docs/roadmap.md)).

Note: secrets now live in a gitignored `.env` (generated by `setup.sh`); the
realm is rendered from `realm.json.tmpl`. `setup.sh` recreates Keycloak each run
so the fresh realm (and its client secrets) is imported.
