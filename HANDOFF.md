...
**Phase 2, next: the operator guide.** A single runbook for operating the
platform — bring-up/teardown, key rotation, policy rollout, incident response,
and the dashboards/alerts an on-call operator needs (see
[`docs/roadmap.md`](docs/roadmap.md)). Then the Phase 2 gate: confirm the threat
model is addressed, dashboards are live, and the secret audit is clean.
