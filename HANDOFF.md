...
- **Status:** Phase 1 **complete**; Phase 2 hardening feature-complete. Done:
  ...
  **secrets out of git/manifests**, **observability** (OpenTelemetry traces with
  ...
  enforcement — ADR-0011), **UI polish**, and the **operator guide**. Next: the
  Phase 2 gate (service metrics + dashboards, then the checklist).
...
open from the backlog: TLS for the remaining in-cluster hops and per-agent
rate/cost limits on the gateway.
