...
- **Status:** Phase 1 **complete**; Phase 2 in progress. Done: the **LLM
  ...
  attributes → collector → Jaeger), and the **policy lifecycle** (versioned
  bundle; every decision names its revision). Next: supply-chain signing,
  operator guide.
...
- **Next: Phase 2** — production hardening. Done so far: the **LLM gateway**
  ...
  collector/Jaeger manifests), and the **policy lifecycle**
  (`scripts/build-bundle.sh` + `docs/policy-lifecycle.md`). Remaining:
  supply-chain signing and the operator guide.

## Immediate next task

**Phase 2, next: supply chain.** Sign images and the policy bundle with cosign
(keyless/OIDC) and enforce signatures with an admission policy (ADR-0010), then
write the operator guide (see [`docs/roadmap.md`](docs/roadmap.md)).
...
(revision stamped into every decision).
