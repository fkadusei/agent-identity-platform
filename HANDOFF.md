...
**Phases 1–3 are complete** — the platform runs end to end and the Phase 3 gate
is closed (Helm chart, CI/CD with an approval gate, managed data stores, sandbox
integrations, adoption guide + runbook).

Next, pick from the open backlog in
[`docs/roadmap.md`](docs/roadmap.md#backlog--known-gaps):

- **Multi-tenant isolation (threat T9)** — a tenant claim enforced in policy and
  the tools, plus a two-tenant test.
- **TLS/mTLS everywhere** — only the agent↔gateway hop is mTLS today.
