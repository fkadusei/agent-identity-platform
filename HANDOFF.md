...
- **Status:** Phase 1 **complete**; Phase 2 in progress. Done: the **LLM
  gateway** (SPIFFE mTLS; the agent holds no model credential), **secrets out of
  git/manifests**, and **observability** (OpenTelemetry traces with identity
  attributes → collector → Jaeger). Next: policy lifecycle, supply-chain
...
- **Next: Phase 2** — production hardening. Done so far: the **LLM gateway**
  ...
  `docs/secrets.md`), and **observability** (`app/common/telemetry.py` + the
  collector/Jaeger manifests in `deploy/kind/manifests/observability/`).
  Remaining: policy lifecycle, SBOM/signing, and the operator guide.

## Immediate next task

**Phase 2, next: policy lifecycle.** Version the Rego bundle and serve it as a
signed/versioned bundle with staged rollout, plus structured decision logs that
record the bundle revision with each decision. Then supply-chain signing
(cosign keyless + admission policy) and the operator guide (see
