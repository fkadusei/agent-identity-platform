...
- **Status:** Phase 1 **complete**; **Phase 2 complete** — the hardening gate
  passed 2026-09-12 (threat model addressed, traces + dashboards live, secret
  audit clean, all attacks blocked). Delivered in Phase 2: the **LLM gateway**
  (SPIFFE mTLS; the agent holds no model credential), **secrets out of
  git/manifests**, **observability** (OpenTelemetry traces with identity
  attributes → collector → Jaeger, plus platform metrics → Prometheus → a
...
remaining in-cluster hops, and per-agent rate/cost limits on the gateway.
