...
| A component is `Running` but never `Ready`, and its UI is dead | a **hung** process — a readiness probe is a signal, not a restart | `kubectl -n agent-platform rollout restart deploy/<name>`. Jaeger carries a liveness probe since S7b and self-heals (it hung for 31h and parked `setup.sh`); the rest of the stack does not yet — see [`backlog.md`](backlog.md) |
